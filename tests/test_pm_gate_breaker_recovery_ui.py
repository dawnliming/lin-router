...
const models = {
  'm-breaker': { id: 'm-breaker', name: 'breaker', group_id: 'g1', usable: false, health_state: 'breaker_open', consecutive_failures: 7, breaker_until: now + 300, breaker_reason: 'redacted_sha256:1234,bytes:9' },
  'm-probe': { id: 'm-probe', name: 'probe', group_id: 'g1', usable: false, health_state: 'half_open_probe', consecutive_failures: 7, breaker_until: now + 300 },
  'm-manual': { id: 'm-manual', name: 'manual', group_id: 'g1', usable: false, disabled_by_user: true, health_state: 'breaker_open', consecutive_failures: 7, breaker_until: now + 300 },
...
if (!breakerModel.includes('连续失败') || !breakerModel.includes('7 次')) throw new Error('failure count missing');
